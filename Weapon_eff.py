...
atk_origin = st.sidebar.number_input("기초 공격력 (공격 보너스 제외 약 1400)", min_value=500.0, max_value=3000.0, value=1454.0, step=1.0, format="%.0f")
...
st.write(f"관리실 공격력: {(atk_origin+atk_val+wep_atk_A)*(1+(atk_bonus+atk_per+wepA_ak)*0.01):.0f}")
...
final_ct_A = buff_y + wepA_ct
final_ct_B = buff_y + wepB_ct
